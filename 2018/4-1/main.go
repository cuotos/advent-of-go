...
func main() {
	...
	data, err := ioutil.ReadFile("2018/4-1/input")
	if err != nil {
		panic(err)
	}
	...
	guardWhoSleptTheMost := sleptMost(events)
	min := timeMostLikelyAsleep(events, guardWhoSleptTheMost)

	fmt.Println(guardWhoSleptTheMost * min)

}

func timeMostLikelyAsleep(events []*event, gid int) int {
	var minutes = make(map[int]int, 60)

	for i, e := range events {
		if e.gid != gid || e.eventType != wakes {
			continue
		} else {
